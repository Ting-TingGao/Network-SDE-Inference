...
for t = 1:T
    for i = 1:n1
        ...
        x_i(t+1,3*i-2) = x_i(t,3*i-2)+(10.07386531*x_i(t,3*i-1)-12.12511225*x_i(t,3*i-2)+1.0015744*tmp)*dt+x(t,3*i-2)/sqrt(gamma)*s(t,3*i-2);
        x_i(t+1,3*i-1) = x_i(t,3*i-1)+((27.60717738-0.98592823*x_i(t,3*i))*x_i(t,3*i-2)-2.95479691*x_i(t,3*i-1))*dt+(28-x(t,3*i))/sqrt(gamma)*s(t,3*i-1);
        x_i(t+1,3*i) = x_i(t,3*i)+(1.00233568*x_i(t,3*i-2)*x_i(t,3*i-1)-6.67988203*x_i(t,3*i))*dt+x(t,3*i-1)/sqrt(gamma)*s(t,3*i);
    end
end
